function Sampler = StickFK_Sampler(data_info, model_setup, chain_start)
...
%         end
        ...
        Pos = normpdf(repmat(dep,1,n_cluster),repmat(mu,n1,1),1./sqrt(repmat(phi(i-1,:),n1,1)));
        Pos = Pos.*omega;
...
%         mu(i,:)=normrnd(nu,1./sqrt(xi));
        ...
        phi_new_b = err2(repmat(dep,1,n_cluster).*Im, Im.*repmat(mu,n1,1));
        ...
        post_likelihood(i) = sum(log(normpdf(dep, Im*mu',1./sqrt(Im*phi(i-1,:)'))));
    end
%     if ishandle(hdl)
%         delete(hdl);
%     end
    Sampler = struct('factor', factor, 'level', level, ...
                     'phi', phi, 'factor_xi', ...
                     factor_xi, 'level_xi', level_xi, 'post_likelihood',post_likelihood);
end